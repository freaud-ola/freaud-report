...
def load_data():
    print("[1/6] 加载数据...")
    df_meta    = pd.read_excel(os.path.join(META_DIR, '作业信息-20260329.xlsx'))
    df_history = pd.read_excel(os.path.join(META_DIR, '调度历史日志-20260329.xlsx'))
    df_res     = pd.read_excel(os.path.join(META_DIR, 'hive 调度资源使用日志-20260329.xlsx'))
    df_sql     = pd.read_excel(os.path.join(META_DIR, 'etl 任务sql代码-20260329.xlsx'))
...
<div class="section" id="sec-opt">
  ...
  <div class="section-body">
    ...
    <div class="ctrl-row" style="margin-top:12px;display:flex;align-items:center;flex-wrap:wrap;gap:10px">
      <label style="font-size:.85rem;color:#555;white-space:nowrap">筛选库名：</label>
      <select id="gap-db-filter" class="form-select form-select-sm" style="width:auto;min-width:180px">
        <option value="">全部</option>
      </select>
      <span style="font-size:.82rem;color:#888;margin-left:auto">共 <strong id="gap-cnt">—</strong> 条记录（上游空闲 &gt;60 分钟）</span>
    </div>
...
(function(){
  ...
  $(document).ready(function(){
    var dt = $('#opt-tbl').DataTable({
      pageLength: 50,
      lengthMenu: [[20,50,100,200,-1],[20,50,100,200,'全部']],
      order: [[7,'desc']],
    ...
    gapData.forEach(function(r){
      var db = r['\u5e93\u540d'] || r['库名'] || '';
      if(db && dbSet.indexOf(db) < 0) dbSet.push(db);
    });
    dbSet.sort();
    var sel = document.getElementById('gap-db-filter');
    dbSet.forEach(function(db){
      var o = document.createElement('option');
      o.value = db; o.textContent = db;
      sel.appendChild(o);
    });

    // 筛选框 → DataTables 第2列（库名）精确过滤
    sel.addEventListener('change', function(){
      var val = this.value;
      dt.column(2).search(val ? '^' + val.replace(/[.*+?^${}()|[\]\\]/g,'\\$&') + '$' : '', true, false).draw();
    });
  });
})();
...
# ─────────────────────────────────────────────
# 导出 summary.json（供对比页面使用）
# ─────────────────────────────────────────────
def write_summary_json(stats, progress, res_data, gap_df, queue_data, ads_pivot):
    hours = list(range(1, 11))
    h_cols = ['h%d' % h for h in hours]

    sla_ok   = int(len(progress[progress['h9'] >= 90]))
    sla_fail = int(len(progress[progress['h10'] < 80]))
    sla_fail_data = progress[progress['h10'] < 80][['db_name','layer','h9','h10']].to_dict('records')

    layer_avg = progress.groupby('layer')[h_cols].mean().reset_index().round(2).to_dict('records')

    # 按库+层的小时完成率明细（供对比页面下钻使用）
    chart_data = {}
    for _, row in progress.iterrows():
        db = row['db_name']
        if db not in chart_data:
            chart_data[db] = {}
        chart_data[db][row['layer']] = [round(row['h%d' % h], 2) for h in hours]
    db_list = sorted(chart_data.keys())

    queue_wait_avg = [{'hour': d['hour'], 'avg_queue': round(d['avg_queue'], 2)} for d in queue_data]

    gap_30 = int(len(gap_df[gap_df['最小空闲(分)'] >= 30]))

    summary = {
        'date_label':       os.path.basename(BASE_DIR),
        'total_jobs':       stats['total_jobs'],
        'dep_rate':         stats['dep_rate'],
        'retry_gt0':        stats['retry_gt0'],
        'self_rely':        stats['self_rely'],
        'sla_ok':           sla_ok,
        'sla_fail':         sla_fail,
        'sla_fail_data':    sla_fail_data,
        'type_dist':        stats['type_dist'],
        'layer_dist':       stats['layer_dist'],
        'cron_hour_dist':   {str(k): v for k, v in sorted(stats['cron_hour_dist'].items())},
        'layer_avg':        layer_avg,
        'chart_data':       chart_data,
        'db_list':          db_list,
        'queue_dist':       res_data['queue_dist'],
        'hourly_concurrent': {str(k): v for k, v in sorted(res_data['hourly_concurrent'].items())},
        'queue_wait_avg':   queue_wait_avg,
        'db_res':           res_data['db_res'][:15],
        'gap_30_count':     gap_30,
        'ads_total':        ads_pivot[3]['total_pivot'],
        'ads_high':         ads_pivot[3]['high_risk'],
        'ads_mid':          ads_pivot[3]['mid_risk'],
        'ads_low':          ads_pivot[3]['low_risk'],
        'ads_max':          ads_pivot[3]['max_downstream'],
    }
    out = os.path.join(BASE_DIR, 'summary.json')
...
def main():
    ...
    write_summary_json(stats, progress, res_data, gap_df, queue_data, ads_pivot)

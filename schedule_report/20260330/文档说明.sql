...
-- 查询 sql
SELECT  * from  dws_meta.dts_sch_job_df where dt='2026-03-29'  AND enabled=1 and deleted=0 
...
SELECT  * from  dws_meta.dwd_sch_history_job_di where dt>='2026-03-27' and dt<='2026-03-29' and status = 'SUCCESSFUL' and execute_type='AUTO'
--------------------------------------------------------------------------------------
-- 资源使用情况表结构
...
select * from dws_meta.dwd_sch_history_application_di where dt>='2026-03-27' and dt<='2026-03-29' and status='SUCCESSFUL' 
--------------------------------------------------------------------------------------
-- etl 任务 sql
SELECT     
            dt
  ...
  from(
    SELECT id,name,version_id
     ...
     WHERE
        dt = '2026-03-29' AND job_type='ETL' AND enabled=1 and deleted=0
     ) m JOIN  dws_meta.dts_sch_code_df n on n.dt='2026-03-29' and n.job_id=m.id and m.version_id=n.version_id
